    ...
    <div className="animate-fadeUp">
      ...
      <Glass pad={false} className="divide-y divide-white/[0.05] !p-0">
        {(data?.strategies ?? []).map((s, idx) => (
          <div key={s.id}>
            <button onClick={() => loadVersions(s.id)} className="tap flex w-full items-center gap-4 px-5 py-5 text-left transition hover:bg-white/[0.015]">
              {s.status === "ACTIVE" ? <GlowDot tone="pos" /> : s.status === "PAUSED" ? <GlowDot tone="warn" pulse={false} /> : <span className="h-[7px] w-[7px] rounded-full bg-white/15" />}
                  ...
                  {(versions[s.id] ?? []).length > 0 && (
                    <div className="mt-4">
                      ...
                      <div className="space-y-1.5">
                        {(versions[s.id] ?? []).map((v) => (
                          <div key={v.id} className="flex items-center justify-between rounded-xl border border-white/[0.05] bg-white/[0.02] px-4 py-3">
                            <div className="flex items-center gap-3">
                              {v.active ? <GlowDot tone="pos" size={6} pulse={false} /> : <span className="h-[6px] w-[6px] rounded-full bg-white/15" />}
