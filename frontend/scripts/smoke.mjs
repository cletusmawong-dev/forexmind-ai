...
global.fetch = (url, opts) => {
  const u = String(url);
  return u.startsWith("http") ? realFetch(u, opts) : realFetch("http://127.0.0.1:8000" + u, opts);
};
...
try {
  ...
  console.log("renders home content:", /FOREXMIND|goal|Market watch|AI activity/i.test(text));
  console.log("renders balance:", /\$[\d,]+\.\d{2}/.test(text));
